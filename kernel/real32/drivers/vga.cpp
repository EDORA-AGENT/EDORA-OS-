...
static void boot_delay()
{
    for (volatile unsigned int count = 0; count < 900000; ++count)
    {
    }
}
...
void vga_write_at(int x, int y, char c)
{
    ...
    void vga_boot_splash()
    {
        ...
        vga_set_color(7, 1);
        vga_set_cursor(22, 9);
        vga_print("Starting system...");

        vga_set_cursor(14, 12);
        vga_print("[");
        ...
        {
            vga_set_color(10, 1);
            vga_set_cursor(16, 15);
            vga_print("[ OK ] ");
            vga_print(boot_stages[stage]);

            vga_set_cursor(15 + stage * 10, 12);
...
void vga_terminal_start()
{
    ...
    vga_print(
        "Command-line startup interface"
    );
